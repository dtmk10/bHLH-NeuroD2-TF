...
dnds <- read.delim("~/documents/UPFClasses/PGB_Project/human_chimp_dnds_02112205.txt", sep="\t", header=TRUE)
bhlh <- read.delim("~/documents/UPFClasses/PGB_Project/HumanTFs_DBD.txt", sep="\t", header=TRUE)

genome <- read.delim("~/documents/UPFClasses/PGB_Project/whole_genome_chimp_human.txt", sep="\t", header=TRUE)
...
genome$set <- "Genome-wide"
bhlh_dnds$set <- "bHLH"

combined <- bind_rows(genome, bhlh_dnds)

ggplot(combined, aes(x = omega, fill = set)) +
  geom_density(alpha = 0.4) +
  geom_vline(xintercept = neurod2$omega, color = "red", linewidth = 1.2) +
  labs(title = "Distribution of dN/dS (ω): bHLH TFs vs Genome",
       x = "ω (dN/dS)", y = "Density") +
  theme_minimal(base_size = 12)
